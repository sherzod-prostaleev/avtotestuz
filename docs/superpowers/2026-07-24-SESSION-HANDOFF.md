...
| M3 | Super Admin | **partial** M3-0…M3-5 (auth, users, content, payments, CMS chrome, monitoring health/metrics/jobs); analytics/flags next; thin ops stubs deprecated at `/ops/*` |
...
```text
Inventory U-xx: U-45 M3-6 Analytics tiles (honest DB aggregates),
then M3-7 flags OR broadcast stub, then U-40 B2B.
