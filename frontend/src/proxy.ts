...
export default function proxy(request: NextRequest) {
  ...
  // Legacy Ops accepted a shared token in sessionStorage. Its capabilities
  // now live behind the normal admin HttpOnly-cookie and RBAC boundary.
  if (pathname === "/ops" || pathname.startsWith("/ops/")) {
    const legacyPage = pathname.split("/")[2] ?? "health";
    const adminPage = LEGACY_OPS_REDIRECTS[legacyPage] ?? "monitoring/health";
    return NextResponse.redirect(new URL(`/${locale}/admin/${adminPage}`, request.url));
  }
