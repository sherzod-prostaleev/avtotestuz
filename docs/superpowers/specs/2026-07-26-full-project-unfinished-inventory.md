...
| **U-41** | P3 | M7 | **Observability (metrics, tracing, alerting)** | **partial** | — | `/healthz` + `/readyz` + process-local `GET /metrics` (request counters by status class, uptime). FE ops health aggregates all three. Still no Prometheus/Sentry/tracing/alerting. |
...
| **U-45** | P3 | M3 | **Super Admin entire vertical** | partial | Most product features | **SoT locked** (`2026-07-26-m3-super-admin-control-center.md`). **M3-0 landed:** `admin_user`+RBAC mig `0030`, `/admin/v1` auth, `/{locale}/admin` shell+login, `make seed-admin`. **M3-1 landed:** `/admin/v1/users` list/detail/block/sessions + `/{locale}/admin/users` (+ detail), audit writes. **M3-2 landed:** `/admin/v1/content/questions` (+ detail, soft validation patch) + explanations queue/verify (`content.verify` mig `0031`) + `/{locale}/admin/content/questions` (+ detail) / `explanations`. **M3-3 landed:** `/admin/v1/payments/transactions` (+ detail, filters) + `/admin/v1/payments/providers` kill-switch (admin auth) + `/{locale}/admin/payments/transactions` (+ detail) / `providers` / `refunds` (docs only — no outbound Payme/Click refund invent). Ops stubs still under `/{locale}/ops/*`. Next: M3-4 CMS, then monitoring. |
