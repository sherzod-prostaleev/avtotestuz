...
| **U-11** | P1 | BE/FE | **M4-08 Web push / campaigns** | **partial** | VAPID keys (ops) | Foundation + test-path hardening (60s cooldown, payload sanitize) + `cmd/pushdigest` dry-run cron stub. Campaigns/admin broadcast / real FSRS digest `-send` still open; needs `VAPID_*` in env to deliver. |
...
| **U-20** | P2 | FE | **N4 chrome tech debt** | partial | U-19 | Premium mobile sticky buy CTA done (popular/first tariff). Remaining: Stats page-level due sticky CTA, raw-color leftovers (provider picker dots), a11y/`no-img-element` notes. |
...
| **U-38** | P3 | M6 | **PWA foundation (manifest, SW, install)** | **partial** | — | Manifest + early SW register + appleWebApp meta done (shares `sw.js`). Install UX polish / PNG icons still optional. |
