...
| **U-45** | P3 | M3 | **Super Admin entire vertical** | partial | Most product features | **SoT locked** (`2026-07-26-m3-super-admin-control-center.md`). **M3-0 landed:** `admin_user`+RBAC mig `0030`, `/admin/v1` auth, `/{locale}/admin` shell+login, `make seed-admin`. **M3-1 landed:** `/admin/v1/users` list/detail/block/sessions + `/{locale}/admin/users` (+ detail), audit writes. Ops stubs still under `/{locale}/ops/*`. Next: M3-2 content, then payments/CMS/monitoring. |
