    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <header className="flex h-14 items-center justify-between px-4 border-b border-border/40">
        <Link href={`/${locale}`} className="flex items-center gap-2 font-display text-lg font-bold">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-accent text-white shadow-3d text-sm">🚗</span>
          <span className="bg-gradient-to-r from-accent to-indigo-500 bg-clip-text text-transparent">AvtoTest</span>
        </Link>
        <ThemeToggle />
      </header>
      ...
      <main className="flex flex-1 items-center justify-center p-4">
        <Card className="glass-card w-full max-w-sm p-8 space-y-6 shadow-2xl border-accent/20 animate-fade-in">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="font-display text-2xl font-extrabold tracking-tight">Kirish</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <div className="rounded-xl border border-danger/50 bg-danger/10 p-3 text-xs font-semibold text-danger">
            ...
            <Button type="submit" variant="game" size="lg" className="w-full py-3 text-sm font-extrabold" disabled={phone.length !== 9 || submitting}>
              {submitting ? "Kutilmoqda..." : t("continue")}
            </Button>
          </form>

          <div className="flex items-center justify-center gap-1.5 text-[11px] text-muted-foreground">
            <ShieldCheck className="h-3.5 w-3.5 text-success" />
            <span>Xavfsiz SMS orqali tasdiqlash</span>
          </div>
        </Card>
      </main>
    </div>
