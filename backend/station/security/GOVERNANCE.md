...
| --- | --- | --- |
| All `GO-*` IDs other than `GO-2026-5024` | They are reachable findings in Go 1.20.14's end-of-life standard library. Their fixed releases require leaving the Go 1.20 family. | Retire Windows 7 and rebuild the client on a maintained Go release, or provide a separately supported Win7-compatible client whose binary scan no longer reports the ID. |
...
sealing/unsealing and registry hardware-ID behavior, and run the station's
owned-HTTPS connection path. Linux tests and cross-compilation do not prove
this.
