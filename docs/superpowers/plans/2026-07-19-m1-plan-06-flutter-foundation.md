...
**Known environment bug — use `dart analyze`, not `flutter analyze`.** Discovered during Task 1: `flutter analyze` crashes (`FormatException: Unterminated string`) specifically because this repo's path contains Cyrillic characters (`Рабочий стол`) — the analysis-server LSP handshake computes its `Content-Length` header from UTF-16 code-unit count instead of UTF-8 byte length, so the header undercounts whenever the embedded workspace path has non-ASCII characters, truncating the JSON payload. Partially patched at `~/.local/flutter/packages/flutter_tools/lib/src/dart/analysis.dart` (the flutter_tools→analysis_server direction now computes real UTF-8 byte length), but the bundled `analysis_server` package's own channel implementation has the same bug in the reverse direction and isn't editable source in this SDK distribution — so the crash persists. **Every task in this plan must use `dart analyze` instead of `flutter analyze`** for local verification (confirmed equivalent signal — same analyzer engine, "No issues found!" on this project) — CI is unaffected since GitHub Actions checkout paths are ASCII-only, so `.github/workflows/ci.yml`'s `flutter analyze` step stays as-is.

**Known environment quirk — always run `flutter test --concurrency=1`.** Discovered during Task 2's re-review: bare `flutter test` (no args, default concurrency) silently misbehaves on this repo's Cyrillic+space path — it can truncate after the first file or misattribute test names across parallel workers, giving a false impression of what actually ran. `flutter test --concurrency=1` (optionally naming files explicitly) gives a clean, correctly-attributed run. **Every task in this plan must use `flutter test --concurrency=1`, not bare `flutter test`,** for local verification. CI is unaffected (ASCII paths), so `.github/workflows/ci.yml`'s `flutter test` step stays as-is.
...
**Files:** create `app/lib/features/auth/presentation/phone_entry_screen.dart`, `app/lib/features/auth/presentation/otp_verify_screen.dart`, and matching widget tests; modify `app/lib/app/router.dart` (add `/login`, `/login/verify` routes + `redirect:` guard); modify `app/lib/app/app.dart` or `app/lib/main.dart` (real DI wiring, see below — a gap identified while dispatching this task, not present in the original brief text).

**Real DI wiring (closes a gap in this plan — Task 6 left `authRepositoryProvider` as `throw UnimplementedError`, a deliberate seam for exactly this task):** at the app root, override `authRepositoryProvider` with a real instance built from Task 2's `buildDio`/`SharedPrefsTokenStorage`/`AuthApi`: construct a `SharedPrefsTokenStorage`, call `buildDio(baseUrl: ..., tokenStorage: ..., onSessionExpired: () async { /* route to /login, e.g. by invalidating authControllerProvider or navigating directly */ })`, wrap in `AuthApi`, wrap in `AuthRepository`, and provide it via `ProviderScope(overrides: [authRepositoryProvider.overrideWithValue(...)])` in `main.dart` (or inside `AvtotestApp` if constructing there is cleaner — your call, but it must happen before `authControllerProvider` is ever read, i.e. before the router/screens mount). The `baseUrl` should come from `String.fromEnvironment('API_BASE_URL', defaultValue: 'http://localhost:8090/api/v1')` (matches this plan's Global Constraints on the dev backend's port and the `--dart-define=API_BASE_URL=...` override convention used in Task 9's live verification). Without this wiring, the app would crash immediately on first read of `authControllerProvider` — this must ship in this task, not be deferred further, since this is the first task that makes auth UI-reachable in a real running app.
