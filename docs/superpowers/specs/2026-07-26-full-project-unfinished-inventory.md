...
| **U-10** | P1 | Bot | **M4-07 TG on-demand quiz + DM digest** | **done** | M4-06 ✅ | `/quiz` image+inline sessions (groups/DM); soft CTA; `/unlink`; optional `cmd/tgdigest`. Not daily group spam. See m4-07-telegram-quiz-growth.md. |
...
| **U-17** | P1 | Docs/FE | **Landing footer real contacts** | **done** | — | `site_settings.contacts` + public `GET /site/contacts`. **Admin home:** `/{locale}/admin/cms/chrome` + `/admin/v1/cms/contacts` (M3-4). Ops `/ops/contacts` deprecated bridge. Empty fields keep i18n placeholders. |
...
| **U-35** | P2 | FE | **Grand Mock “certificate”** | **partial** | M2-07 ✅ | Persist + public page + **Web Share/clipboard + print/PDF via browser**. Admin-issued credential PDF still open. |
...
| **U-45** | P3 | M3 | **Super Admin entire vertical** | partial | Most product features | **SoT locked**. **M3-0…M3-7 + audit + broadcast + limits + inbox + home/chrome CMS + legal CMS (`site_settings.legal` + `/cms/legal` + public `/site/legal`) + ops feed/alerts + investors stub.** Deeper content studios (signs/tickets), runtime config, RBAC UI still open. |
